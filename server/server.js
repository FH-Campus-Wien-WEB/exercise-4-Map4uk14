...
app.put("/movies/:imdbID", requireLogin, function (req, res) {
  ...
  if (!exists) {
    ...
    fetch(`https://www.omdbapi.com/?apikey=${config.omdbApiKey}&i=${imdbID}`, { signal: controller.signal })
      .then(omdbRes => {
        clearTimeout(timeoutId);
        if (!omdbRes.ok) {
          res.sendStatus(omdbRes.status);
          throw new Error(`OMDb returned ${omdbRes.status}`);
        }
        return omdbRes.json();
      })
      .then(omdbData => {
